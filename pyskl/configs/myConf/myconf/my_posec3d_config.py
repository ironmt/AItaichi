...
ann_file = '/disks/disk6/private/ldr/AItaichi/train/train.pkl'
left_kp = [1, 3, 5, 7, 9, 11, 13, 15]
right_kp = [2, 4, 6, 8, 10, 12, 14, 16]
train_pipeline = [
    dict(type='UniformSampleFrames', clip_len=200),
...
val_pipeline = [
    dict(type='UniformSampleFrames', clip_len=200, num_clips=1),
    dict(type='PoseDecode'),
    dict(type='PoseCompact', hw_ratio=1., allow_imgpad=True),
...
test_pipeline = [
    dict(type='UniformSampleFrames', clip_len=200, num_clips=10),
    dict(type='PoseDecode'),
    dict(type='PoseCompact', hw_ratio=1., allow_imgpad=True),
    dict(type='Resize', scale=(64, 64), keep_ratio=False),
...
# optimizer
optimizer = dict(type='SGD', lr=1e-4, momentum=0.9, weight_decay=0.0003)
...
work_dir = f'./work_dirs/posec3d/c3d_light_taichi/clip200_lr1e-3_joint_{current_datetime}'
