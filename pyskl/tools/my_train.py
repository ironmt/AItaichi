...
def plot_training_curve(train_losses, val_losses, output_dir):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(osp.join(output_dir, 'training_curve.png'))
    plt.close()
...
def main():
    ...
    # 修改 train_model 以记录训练和验证损失
    train_losses, val_losses = train_model(model, datasets, cfg, validate=args.validate, test=test_option, timestamp=timestamp, meta=meta)
    dist.barrier()
    ...
    # 在训练结束后绘制曲线
    if rank == 0:
        plot_training_curve(train_losses, val_losses, cfg.work_dir)
